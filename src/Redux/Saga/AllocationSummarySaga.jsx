...
import {
  ...
  postMAPRVSuccess, postMAPRVError,
...
function* fetchMassAprv(action) {
  try {
    const response = yield call(axiosCall, "POST", API.ASMAPRV, action.payload);
    if (response?.status == 200) {
      yield put(postMAPRVSuccess({ mAprvData: response?.data }));
    } else {
      yield put(postMAPRVError(response?.data?.message));
    }
  } catch (e) {
    yield put(postMAPRVError(e.message));
  }
}

export function* MAprvData() {
  yield takeLatest(actions.POST_MAPRV_REQUEST, fetchMassAprv);
}