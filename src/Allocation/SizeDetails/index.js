...
const SizeDetails = ({ ApproveFreeseCheck, allocNoData, setTab, setOpenDialog, setDialogData, ScreenNameAllocDetails, sizeDData,
    setsizeDetailsScreen, setReload, }) => {
    ...
    console.log("Testing:1: ", check, isSubmit, allocNoData,SizeDetailsData?.data?.sizeHeaderDetailsData)
    ...
    useEffect(() => {
        if (
            ...
            if (SizeDetailsData?.data?.sizeHeaderDetailsData?.status === 200) {
                setSizeHeaderDetailsData([]);
                setTableSizeHeaderDetailsData([]);
                setSizeHeaderDetailsData(serializedata(SizeDetailsData?.data?.sizeHeaderDetailsData?.message));
                setTableSizeHeaderDetailsData(serializedata(SizeDetailsData?.data?.sizeHeaderDetailsData?.message));
                var Alloc_qty = 0
                SizeDetailsData?.data?.sizeHeaderDetailsData?.message.map(obj => {
                    Alloc_qty = Alloc_qty + obj.ALLOC_QTY
                })
                setTotalAllocQty(Alloc_qty)
                var check = false;
                console.log("Testing:2: ", isSubmit, allocNoData, SizeDetailsData?.data?.sizeHeaderDetailsData)

                if (SizeDetailsData?.data?.sizeHeaderDetailsData?.message.length > 0) {
                    if (isSubmit) {
                        dispatch(postSIZEDETAILSRequest([SizeDetailsData?.data?.sizeHeaderDetailsData?.message[0]]));
                        setLoadCheck(true);
                        setSubmit(false)
                    } else {
                        setLoadCheck(false);
            ...
            else if (SizeDetailsData?.data?.sizeHeaderDetailsData?.status === 500) {
                setLoadCheck(false);
                if (typeof ScreenNameAllocDetails === "undefined") {
                    setTab('1');
                } else {
                ...
                setSizeTempDataUpdate([]);
                setOpenDialog(true);
                if (String(SizeDetailsData?.data?.sizeHeaderDetailsData?.message).length > 0) { setDialogData(String(SizeDetailsData?.data?.sizeHeaderDetailsData?.message)); }
                else { setDialogData("Please Re-calculate"); }
            }
            if (closeOnBlur === "OK") {
                setCloseOnBlur("");
                handleOk();
            ...
            SizeDetailsData.data.sizeHeaderDetailsData.status = 0
        }
        if (
            SizeDetailsData?.data?.sizeDetailsData
            // && Array.isArray(SizeDetailsData?.data?.sizeDetailsData)
        ) {
            if (SizeDetailsData?.data?.sizeDetailsData?.status === 200) {
                if (SizeDetailsData?.data?.sizeDetailsData?.message[0].length > 0) {
                    const df_data1 = SizeDetailsData?.data?.sizeDetailsData?.message[0]
                    const df_data2 = SizeDetailsData?.data?.sizeDetailsData?.message[1]
                    setSizeTempCheck(true)
                    setLoading(false);
                    setLoadCheck(false);
                    setSizeDetailsData1(SizeDetailsData?.data?.sizeDetailsData?.message)
            ...
            } else if (SizeDetailsData?.data?.sizeDetailsData?.status === 500) {
                setHeadList([]);
                setSizeTempCheck(false);
                setLoadCheck(false);
                setSizeDetailsData1([]);
                ...
                if (String(SizeDetailsData?.data?.sizeDetailsData?.message).length > 0) { setDialogDataSiD(String(SizeDetailsData?.data?.sizeDetailsData?.message)); }
                else { setDialogDataSiD("No data found.") }
                // set()
            }
            SizeDetailsData.data.sizeDetailsData.status = 0
        }
        if (
                            ...
                            <div className={AllocDetailsClasses.float_child}>
                                ...
                                <Button
                                    sx={{
                                        ...
                                        width: "100px", marginLeft: "5px", marginTop: "2px", '&:hover': {
