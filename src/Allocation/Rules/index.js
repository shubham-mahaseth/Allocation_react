...
import {
  ...
  /* RL Template Starts */
  postRULESAVETEMPLATERequest,
  postRLLOCSAVETEMPLATERequest,
  getRULETEMPLATENAMERequest,
  getRLLOCTEMPLATENAMERequest,
  postFETCHRULETEMPLATERequest,
  postFETCHRLLOCTEMPLATERequest,
  /* RL Template ends */
...
const RulesAndLocation = ({ allocNoData, tab, setTab, setIsValidQtyLimits, setRTabCond, setDisCond,
  ...
  const initialLeftRules = {
    ...
    TEMPLATE_NO: "",
    CASCADE_IND: "N",
    USE_RULE_LEVEL_ON_HAND_IND: "Y",
    INCLUDE_CLEARANCE_STOCK_IND: "N",
    INCLUDE_INV_IN_MIN_IND: "Y",
    INCLUDE_INV_IN_MAX_IND: "Y",
    IWOS_WEEKS: "",
    WEEKS_FUTURE: "",
    CORPORATE_RULE_ID: "",
  ...
  if (retreieveRLdataCheck && rtvrldata.length > 0) {
    ...
    const temp = rtvrldata[1].slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter(row => row !== undefined);
 ...
 */
  ...
  //*//*//* RL Template useStates Starts *//*//*//
  const [ruleTemplateCheck, setRuleTemplateCheck] = useState(false);
  const [ruleTemplateName, setRuleTemplateName] = useState("");

  const [locInputTemplateData, setLocInputTemplateData] = useState("");
  const [locTemplateName, setLocTemplateName] = useState("");
  const [locTemplateCheck, setLocTemplateCheck] = useState(false);

  const [ruleTemplateNameData, setRuleTemplateNameData] = useState([]);
  const [locTemplateNameData, setLocTemplateNameData] = useState([]);

  const [fetchRuleTemplateData, setFetchRuleTemplateData] = useState([]);
  const [fetchLocTemplateData, setFetchLocTemplateData] = useState([]);
  //*//*//* RL Template useStates ends *//*//*// 
  ...
  */
  ...
  const handlesubmitRules = () => {
    if (totalData.length > 0) {
      if (String(leftContData.RULE_TYPE) === "M") {
        setLoading(true);
  ...
  */
  ...
  const RulesSaveTemplate = (e) => {
    setRuleTemplateCheck(true);
  }

  const LocationSaveTemplate = (e) => {
    setLocTemplateCheck(true);
  }

  const HandleApplyRulesTemplate = (e) => {
    if (leftContData.TEMPLATE_NO !== "") {
      dispatch(postFETCHRULETEMPLATERequest([leftContData]));
      setLoadCheck(true);
    }
  }

  const HandleApplyLocsTemplate = (e) => {
    if (locTemplateName !== "") {
  ...
  const RulesSaveTemplateOk = (e) => {
    setRuleTemplateCheck(false);
    if (ruleTemplateName.length === 0) {
      setOpenDialogRL(true);
      setDialogDataRL("Rules Template Name is required*");
    }
    else {
      const temp = {
        TEMPLATE_NO: "",
        ...
        LAST_UPDATE_ID: JSON.parse(localStorage.getItem("userData"))?.username,
        LAST_UPDATE_DATETIME: new Date().toISOString().slice(0, 10),
      }
      dispatch(postRULESAVETEMPLATERequest([{ ...leftContData, ...temp }]));
      setLoadCheck(true);
      setRuleTemplateName("");
    }
  }

  const handleRuleTemplateName = (e) => {
    const trimmedValue = e.target.value.replace(/^\s+/, '');
    setRuleTemplateName(trimmedValue);
  }

  const handleLocInputTemplateData = (e) => {
    let input = e.target.value
    input = input.trim();
    // const trimmedValue = input.replace(/\s+/g, '');
    // const trimmedInput = trimmedValue.replace(/^,+|,+$/g, '').replace(/,+/g, ',');;
    // const validPattern = /^[0-9,]+$/;
    const validPattern = /^(\d+)(,\d+)*$/;
    if (!validPattern.test(input) && input.length > 0) {
      setOpenDialogRL(true);
      setDialogDataRL("Invalid input: Only numbers separated by single commas are allowed. No spaces or multiple commas.");
      setLocInputTemplateData("");
    } else {
      setLocInputTemplateData(input);
    }
  }

  const LocSaveTemplateOk = (e) => {
    if (locTemplateName.length === 0) {
      setOpenDialogRL(true);
      setDialogDataRL("Location Template Name is required*");
    }
    else if (locInputTemplateData.length === 0) {
      setOpenDialogRL(true);
      setDialogDataRL("Locations are required*");
    }
    else {
      const temp = {
        TEMPLATE_NO: "",
        TEMPLATE_NAME: locTemplateName,
        LOCATION: locInputTemplateData,
        CREATE_ID: JSON.parse(localStorage.getItem("userData"))?.username,
        CREATE_DATETIME: new Date().toISOString().slice(0, 10),
        LAST_UPDATE_ID: JSON.parse(localStorage.getItem("userData"))?.username,
        LAST_UPDATE_DATETIME: new Date().toISOString().slice(0, 10),
      }
      dispatch(postRLLOCSAVETEMPLATERequest([temp]));
      setLoadCheck(true);
      setLocTemplateName("");
      setLocTemplateCheck(false);
    }
  }

  const handleLocTemplateName = (e) => {
    const trimmedValue = e.target.value.replace(/^\s+/, '');
    setLocTemplateName(trimmedValue);
  }

  useEffect(() => {
    if (RulesLocationData?.data?.ruleTemplateData) {
      if (RulesLocationData?.data?.ruleTemplateData?.status === 201 || RulesLocationData?.data?.ruleTemplateData?.status === 500) {
        setOpenDialogRL(true);
        setDialogDataRL(RulesLocationData?.data?.ruleTemplateData?.message);
        // RulesLocationData?.data?.ruleTemplateData?.status = 0
        dispatch(getRULETEMPLATENAMERequest([{}]));
      }
      setLoadCheck(false);
    }
    if (RulesLocationData?.data?.fetchRuleTemplateData) {
      if (RulesLocationData?.data?.fetchRuleTemplateData && Array.isArray(RulesLocationData?.data?.fetchRuleTemplateData)) {
        setFetchRuleTemplateData(RulesLocationData?.data?.fetchRuleTemplateData)

        if (RulesLocationData?.data?.fetchRuleTemplateData[0].REGULAR_SALES_IND) {
          setLeftContData((prev) => { return { ...prev, REGULAR_SALES_IND: RulesLocationData?.data?.fetchRuleTemplateData[0].REGULAR_SALES_IND === "N" ? "N" : "Y" }; })
          setCheck1(RulesLocationData?.data?.fetchRuleTemplateData[0].REGULAR_SALES_IND === "N" ? false : true)
        }

        if (RulesLocationData?.data?.fetchRuleTemplateData[0].PROMO_SALES_IND) {
          setLeftContData((prev) => { return { ...prev, PROMO_SALES_IND: RulesLocationData?.data?.fetchRuleTemplateData[0].PROMO_SALES_IND === "N" ? "N" : "Y" }; })
          setCheck2(RulesLocationData?.data?.fetchRuleTemplateData[0].PROMO_SALES_IND === "N" ? false : true)
        }

        if (RulesLocationData?.data?.fetchRuleTemplateData[0].CLEARANCE_SALES_IND) {
          setLeftContData((prev) => { return { ...prev, CLEARANCE_SALES_IND: RulesLocationData?.data?.fetchRuleTemplateData[0].CLEARANCE_SALES_IND === "N" ? "N" : "Y" }; })
          setCheck3(RulesLocationData?.data?.fetchRuleTemplateData[0].CLEARANCE_SALES_IND === "N" ? false : true)
        }

        if (RulesLocationData?.data?.fetchRuleTemplateData[0].SIZE_PROFILE_IND) {
          setLeftContData((prev) => { return { ...prev, SIZE_PROFILE_IND: RulesLocationData?.data?.fetchRuleTemplateData[0].SIZE_PROFILE_IND === "N" ? "N" : "Y" }; })
          setCheck4(RulesLocationData?.data?.fetchRuleTemplateData[0].SIZE_PROFILE_IND === "N" ? false : true)
        }


        if (RulesLocationData?.data?.fetchRuleTemplateData[0].ENFORCE_PRES_MIN_IND) {
          setLeftContData((prev) => { return { ...prev, ENFORCE_PRES_MIN_IND: RulesLocationData?.data?.fetchRuleTemplateData[0].ENFORCE_PRES_MIN_IND === "N" ? "N" : "Y" }; })
          setCheck5(RulesLocationData?.data?.fetchRuleTemplateData[0].ENFORCE_PRES_MIN_IND === "N" ? false : true)
        }

        if (RulesLocationData?.data?.fetchRuleTemplateData[0].WH_STORE_REL_IND) {
          setLeftContData((prev) => { return { ...prev, ENFORCE_WH_RL: RulesLocationData?.data?.fetchRuleTemplateData[0].WH_STORE_REL_IND === "N" ? "N" : "Y" }; });
          setAllocateDefWH(RulesLocationData?.data?.fetchRuleTemplateData[0].WH_STORE_REL_IND === "N" ? false : true);
        }

        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["WEEKS_LAST_YEAR"])) {
          setLeftContData((prev) => { return { ...prev, WEEKS_LAST_YEAR: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["WEEKS_LAST_YEAR"]).length > 0 ? RulesLocationData?.data?.fetchRuleTemplateData[0]["WEEKS_LAST_YEAR"] : "" }; });
          setCheck6(String(RulesLocationData?.data?.fetchRuleTemplateData[0]["WEEKS_LAST_YEAR"]).length > 0 ? true : false);
        }

        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["WEEKS_THIS_YEAR"])) {
          setLeftContData((prev) => { return { ...prev, WEEKS_THIS_YEAR: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["WEEKS_THIS_YEAR"]).length > 0 ? RulesLocationData?.data?.fetchRuleTemplateData[0]["WEEKS_THIS_YEAR"] : "" }; });
          setCheck6(String(RulesLocationData?.data?.fetchRuleTemplateData[0]["WEEKS_THIS_YEAR"]).length > 0 ? true : false);
        }

        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["WEEKS_THIS_YEAR"]).length === 0 && String(RulesLocationData?.data?.fetchRuleTemplateData[0]["WEEKS_LAST_YEAR"]).length === 0) {
          setCheck6(false);
          setLeftContData((prev) => { return { ...prev, WEEKS_THIS_YEAR: "", WEEKS_LAST_YEAR: "", }; })
        }

        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["START_DATE1"]).length > 0 || String(RulesLocationData?.data?.fetchRuleTemplateData[0]["END_DATE1"]).length > 0 ||
          String(RulesLocationData?.data?.fetchRuleTemplateData[0]["START_DATE2"]).length > 0 || String(RulesLocationData?.data?.fetchRuleTemplateData[0]["END_DATE2"]).length > 0) {
          setCheck9(true);
          if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["START_DATE1"])) {
            setLeftContData((prev) => { return { ...prev, START_DATE1: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["START_DATE1"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["START_DATE1"]) : "" }; })
          }

          if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["END_DATE1"])) {
            setLeftContData((prev) => { return { ...prev, END_DATE1: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["END_DATE1"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["END_DATE1"]) : "" }; })
          }

          if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["START_DATE2"])) {
            setLeftContData((prev) => { return { ...prev, START_DATE2: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["START_DATE2"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["START_DATE2"]) : "" }; })
          }

          if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["END_DATE2"])) {
            setLeftContData((prev) => { return { ...prev, END_DATE2: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["END_DATE2"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["END_DATE2"]) : "" }; })
          }
        }
        ...
        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["ON_ORDER_COMMIT_WEEKS"])) {
          setLeftContData((prev) => { return { ...prev, ON_ORDER_COMMIT_WEEKS: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["ON_ORDER_COMMIT_WEEKS"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["ON_ORDER_COMMIT_WEEKS"]) : "" }; })
          setCheck7(String(RulesLocationData?.data?.fetchRuleTemplateData[0]["ON_ORDER_COMMIT_WEEKS"]).length > 0 ? true : false)
        }

        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["ON_ORDER_COMMIT_DATE"])) {
          setLeftContData((prev) => { return { ...prev, ON_ORDER_COMMIT_DATE: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["ON_ORDER_COMMIT_DATE"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["ON_ORDER_COMMIT_DATE"]) : "" }; })
          setCheck8(String(RulesLocationData?.data?.fetchRuleTemplateData[0]["ON_ORDER_COMMIT_DATE"]).length > 0 ? true : false)
        }

        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["RULE_TYPE"]).length > 0) {
          const temp = ruleType.filter(item => item.CODE === String(RulesLocationData?.data?.fetchRuleTemplateData[0]["RULE_TYPE"]))
          if (temp.length > 0) {
            setLeftContData((prev) => { return { ...prev, RULE_TYPE: temp[0].CODE, RULE_TYPE_VAL: temp[0].CODE_DESC, }; })
          }
        }

        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["EXACT_IND"]).length > 0) {
          const temp = Need.filter(item => item.CODE === String(RulesLocationData?.data?.fetchRuleTemplateData[0]["EXACT_IND"]))
          if (temp.length > 0) {
            setLeftContData((prev) => { return { ...prev, EXACT_IND_VAL: temp[0].CODE_DESC, EXACT_IND: temp[0].CODE, }; })
          }
        }

        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["NET_NEED_IND"]).length > 0) {
          const temp = Allocateto.filter(item => item.CODE === String(RulesLocationData?.data?.fetchRuleTemplateData[0]["NET_NEED_IND"]))
          if (temp.length > 0) {
            setLeftContData((prev) => { return { ...prev, NET_NEED_IND_VAL: temp[0].CODE_DESC, NET_NEED_IND: temp[0].CODE, }; })
          }
        }

        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["RULE_LEVEL"]).length > 0) {
          const temp = Hierarchy.filter(item => item.CODE === String(RulesLocationData?.data?.fetchRuleTemplateData[0]["RULE_LEVEL"]))
          if (temp.length > 0) {
            setLeftContData((prev) => { return { ...prev, RULE_LEVEL: temp[0].CODE, RULE_LEVEL_VAL: temp[0].CODE_DESC, }; })
          }
        }

        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["INCLUDE_INV_IN_MIN_IND"])) {
          setLeftContData((prev) => { return { ...prev, INCLUDE_INV_IN_MIN_IND: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["INCLUDE_INV_IN_MIN_IND"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["INCLUDE_INV_IN_MIN_IND"]) : "" }; })
        }
        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["INCLUDE_INV_IN_MAX_IND"])) {
          setLeftContData((prev) => { return { ...prev, INCLUDE_INV_IN_MAX_IND: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["INCLUDE_INV_IN_MAX_IND"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["INCLUDE_INV_IN_MAX_IND"]) : "" }; })
        }
        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["CASCADE_IND"])) {
          setLeftContData((prev) => { return { ...prev, CASCADE_IND: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["CASCADE_IND"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["CASCADE_IND"]) : "" }; })
        }
        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["USE_RULE_LEVEL_ON_HAND_IND"])) {
          setLeftContData((prev) => { return { ...prev, USE_RULE_LEVEL_ON_HAND_IND: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["USE_RULE_LEVEL_ON_HAND_IND"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["USE_RULE_LEVEL_ON_HAND_IND"]) : "" }; })
        }
        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["INCLUDE_CLEARANCE_STOCK_IND"])) {
          setLeftContData((prev) => { return { ...prev, INCLUDE_CLEARANCE_STOCK_IND: String(RulesLocationData?.data?.fetchRuleTemplateData[0]["INCLUDE_CLEARANCE_STOCK_IND"]).length > 0 ? String(RulesLocationData?.data?.fetchRuleTemplateData[0]["INCLUDE_CLEARANCE_STOCK_IND"]) : "" }; })
        }
        if (String(RulesLocationData?.data?.fetchRuleTemplateData[0]["TEMPLATE_NO"]).length > 0) {
          const temp = ruleTemplateNameData.filter(item => item.TEMPLATE_NO === String(RulesLocationData?.data?.fetchRuleTemplateData[0]["TEMPLATE_NO"]))
          if (temp.length > 0) {
      ...
      else if (RulesLocationData?.data?.fetchRuleTemplateData?.status === 500) {
        setOpenDialogRL(true);
        setDialogDataRL(RulesLocationData?.data?.fetchRuleTemplateData?.message);
        // RulesLocationData?.data?.fetchRuleTemplateData?.status = 0
      }
      setLoadCheck(false);
    }

    if (RulesLocationData?.data?.locTemplateData) {
      if (RulesLocationData?.data?.locTemplateData?.status === 201 || RulesLocationData?.data?.locTemplateData?.status === 500) {
        setOpenDialogRL(true);
        setDialogDataRL(RulesLocationData?.data?.locTemplateData?.message);
        dispatch(getRLLOCTEMPLATENAMERequest([{}]));
        // RulesLocationData?.data?.ruleTemplateData?.status = 0
      }
      setLoadCheck(false);
    }
    if (RulesLocationData?.data?.fetchLocTemplateData) {
      if (RulesLocationData?.data?.fetchLocTemplateData && Array.isArray(RulesLocationData?.data?.fetchLocTemplateData)) {
        setFetchLocTemplateData(RulesLocationData?.data?.fetchLocTemplateData)
        console.log("RulesLocationData?.data: ", RulesLocationData?.data, RulesLocationData?.data?.fetchLocTemplateData);
        for (let i = 0; i < RulesLocationData?.data?.fetchLocTemplateData.length; i++) {
          let rec = RulesLocationData?.data?.fetchLocTemplateData[i];
          for (let col in rec) {
        ...
        setTotalData(RulesLocationData?.data?.fetchLocTemplateData);
        setTableLocData(RulesLocationData?.data?.fetchLocTemplateData);
        const temp = RulesLocationData?.data?.fetchLocTemplateData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter(row => row !== undefined);
        setAllData(RulesLocationData?.data?.fetchLocTemplateData);
        setcurrentPageData(temp);
        setcurrentPageRows(temp);
        setLoadCheck(false);
        setPage(0);
      } else if (RulesLocationData?.data?.fetchLocTemplateData?.status === 500) {
        setOpenDialogRL(true);
        setDialogDataRL(RulesLocationData?.data?.fetchLocTemplateData?.message);
        setTotalData([]);
        setTableLocData([]);
        setAllData([]);
        setcurrentPageData([]);
  ...
  const selectRuleTemplate = (val) => {
    if (val) {
      setLeftContData(prev => ({ ...prev, TEMPLATE_NO: val.TEMPLATE_NO }))
    } else {
      setLeftContData(prev => ({ ...prev, TEMPLATE_NO: "" }))
    }
  }
  const selectLocTemplate = (val) => {
    if (val) {
      setLocTemplateName(val.TEMPLATE_NAME);
    } else {
      setLocTemplateName("");
    }
  }
  ...
  */
  ...
  const RulesAndLocationsDrawer = () => (
    ...
    >
      <div>
        <div >
          <Typography style={{ bgcolor: '', fontSize: "16px", fontWeight: "bold", position: "static", padding: "0px" }}>
            Rules & History Range
          </Typography></div>
        <div >
          <Typography style={{ bgcolor: '', fontSize: "14px", fontWeight: "bold", position: "static", padding: "0px" }}>
            Sales History Types:
          </Typography></div>
      </div>
        ...
        <div className={RulesLocationHeaderClasses.header_child1}>
          <Button
            sx={{
              ...
              width: "100px", marginLeft: "10px", marginTop: "10px",
              '&.Mui-disabled': {
                opacity: 0.5,
        ...
        <div className={RulesLocationHeaderClasses.header_child}>
          ...
          <div>
            <Select
              ...
              classNamePrefix="mySelect"
              getOptionLabel={option =>
                `${option.TEMPLATE_NAME.toString()}`}
              getOptionValue={option => option.TEMPLATE_NAME}
              options={ruleTemplateNameData.length > 0 ? ruleTemplateNameData : []}
              isSearchable={true}
              menuPlacement="auto"
              isClearable={true}
              onChange={selectRuleTemplate}
              isDisabled={ApproveFreeseCheck}
              closeMenuOnSelect={true}
              hideSelectedOptions={false}
              styles={styleSelect6}
              components={animatedComponents}
              value={ruleTemplateNameData.filter(obj => leftContData?.TEMPLATE_NO === (obj.TEMPLATE_NO))}
            />
          </div>
        </div>
        ...
        <div className={RulesLocationHeaderClasses.header_child}>
          <div className={RulesLocationHeaderClasses.header_child}>
            <Button
              ...
              disabled={ApproveFreeseCheck}
              onClick={HandleApplyRulesTemplate}
  ...
  const DrawerLocation = () => (
    <Box sx={{ margin: "0px 0px 0px 10px" }}>
      <div className={RulesLocationHeaderClasses.header_child2} >
        <FormControlLabel size="small" sx={{ margin: "0px 0px 0px 0px", padding: "0px", border: "1px solid black" }}
          control={
            <Switch
              disabled={ApproveFreeseCheck}
              size="small" name="AllStoresCheck"
          ...
          <div>
            <Select
              ...
              getOptionLabel={option =>
                `${option.TEMPLATE_NAME.toString()}`}
              getOptionValue={option => option.TEMPLATE_NAME}
              options={locTemplateNameData.length > 0 ? locTemplateNameData : []}
              isSearchable={true}
              menuPlacement="auto"
              isClearable={true}
              onChange={selectLocTemplate}
              isDisabled={ApproveFreeseCheck}
              closeMenuOnSelect={true}
              hideSelectedOptions={false}
              styles={styleSelect6}
              components={animatedComponents}
              value={locTemplateNameData.filter(obj => locTemplateName === obj.TEMPLATE_NAME)}
            />
          </div>
        </div>
        ...
        <div className={RulesLocationHeaderClasses.header_child}>
          <div className={RulesLocationHeaderClasses.header_child}>
            <Button
              ...
              startIcon={<OfflinePinIcon sx={{ padding: 0 }} />}
              variant="contained"
              onClick={HandleApplyLocsTemplate}
              disabled={ApproveFreeseCheck}
            >
              Apply
            </Button>
          ...
          <div className={RulesLocationHeaderClasses.header_child}>
            ...
            <div >
              ...
              <DatePicker
                ...
                selected={convert_Date_picker(leftContData.START_DATE1)}
                onChangeRaw={(event) => {
                  event.preventDefault();
                }}
                maxDate={convert_Date_picker(leftContData.END_DATE1)}
                onChange={(date) => handleDatePicker("START_DATE1", date)}
                dateFormat="MM-dd-yy"
                  ...
                  <TextField
                    ...
                    InputProps={{
                      ...
                      endAdornment: (<>
                        ...
                        {(leftContData?.START_DATE1?.length > 0)
                          && <BsFillEraserFill fontSize="medium"
                            onClick={() => handleDateErase("START_DATE1")} />}
                      </>)
                    }}
                    inputProps={{
                      // min: new Date().toISOString().slice(0, 10),
                      sx: { backgroundColor: '#fff', height: "10px", borderRadius: "5px" }
                    }}
                    disabled={ApproveFreeseCheck}
                  />
            ...
            <div >
              ...
              <DatePicker
                autoComplete="off"
                placeholderText="MM-DD-YY"
                selected={convert_Date_picker(leftContData.START_DATE2)}
                onChangeRaw={(event) => {
                  event.preventDefault();
                }}
                maxDate={convert_Date_picker(leftContData.END_DATE2)}
                onChange={(date) => handleDatePicker("START_DATE2", date)}
                showYearDropdown  // Enable year dropdown
                showMonthDropdown
                scrollableMonthYearDropdown
                scrollableYearDropdown={true}
                ...
                customInput={
                  ...
                  <TextField
                    ...
                    InputProps={{
                      ...
                      endAdornment: (<>
                        <CalendarTodayIcon style={{ fontSize: "11px", margin: "0px 3px 0px 0px" }} />
                        {(leftContData?.START_DATE2?.length > 0)
                          && <BsFillEraserFill fontSize="medium"
                            onClick={() => handleDateErase("START_DATE2")} />}
                      </>)
                    }}
                    inputProps={{
                      // min: new Date().toISOString().slice(0, 10),
          ...
          <div className={RulesLocationHeaderClasses.header_child}>
            ...
            <div>
              ...
              <DatePicker
                autoComplete="off"
                placeholderText="MM-DD-YY"
                selected={convert_Date_picker(leftContData.END_DATE1)}
                onChangeRaw={(event) => {
                  event.preventDefault();
                }}
                minDate={convert_Date_picker(leftContData.START_DATE1)}
                onChange={(date) => handleDatePicker("END_DATE1", date)}
                showYearDropdown  // Enable year dropdown
                showMonthDropdown
                scrollableMonthYearDropdown
                scrollableYearDropdown={true}
                ...
                customInput={
                  ...
                  <TextField
                    ...
                    InputProps={{
                      ...
                      endAdornment: (<>
                        <CalendarTodayIcon style={{ fontSize: "11px", margin: "0px 3px 0px 0px" }} />
                        {(leftContData?.END_DATE1?.length > 0)
                          && <BsFillEraserFill fontSize="medium"
                            onClick={() => handleDateErase("END_DATE1")} />}
                      </>)
                    }}
                    inputProps={{
                      // min: new Date().toISOString().slice(0, 10),
            ...
            <div >
              ...
              <DatePicker
                autoComplete="off"
                placeholderText="MM-DD-YY"
                selected={convert_Date_picker(leftContData.END_DATE2)}
                onChangeRaw={(event) => {
                  event.preventDefault();
                }}

                minDate={convert_Date_picker(leftContData.START_DATE2)}
                onChange={(date) => handleDatePicker("END_DATE2", date)}
                dateFormat="MM-dd-yy"
                disabled={ApproveFreeseCheck}
                showYearDropdown  // Enable year dropdown
                showMonthDropdown
                ...
                customInput={
                  ...
                  <TextField
                    ...
                    InputProps={{
                      ...
                      endAdornment: (<>
                        <CalendarTodayIcon style={{ fontSize: "11px", margin: "0px 3px 0px 0px" }} />
                        {(leftContData?.END_DATE2?.length > 0)
                          && <BsFillEraserFill fontSize="medium"
                            onClick={() => handleDateErase("END_DATE2")} />}
                      </>)
                    }}
                    inputProps={{
                      // min: new Date().toISOString().slice(0, 10),
        ...
        <div>
          ...
          <div>
            ...
            <DatePicker
              autoComplete="off"
              placeholderText="MM-DD-YY"
              selected={convert_Date_picker(leftContData.ON_ORDER_COMMIT_DATE)}
              onChange={(date) => handleDatePicker("ON_ORDER_COMMIT_DATE", date)}
              onChangeRaw={(event) => {
                event.preventDefault();
              }}
              dateFormat="MM-dd-yy"
              ...
              customInput={
                ...
                <TextField
                  ...
                  InputProps={{
                    ...
                    endAdornment: (<>
                      <CalendarTodayIcon style={{ fontSize: "11px", margin: "0px 3px 0px 0px" }} />
                      {(leftContData?.ON_ORDER_COMMIT_DATE?.length > 0)
                        && <BsFillEraserFill fontSize="medium"
                          onClick={() => handleDateErase("ON_ORDER_COMMIT_DATE")} />}
                    </>)
                  }}
                  inputProps={{
                    // min: new Date().toISOString().slice(0, 10),
  ...
  */
  ...
  useEffect(() => {
    if (
      ...
      setOpenDialogRL(true);
      setDialogDataRL(String(RulesLocationData?.data?.updateChangeWeightsRL?.message));
      setLoadCheck(false);
      setLoading(false);
    } else if (
      ...
      if (countAddCheck === 0) {
        setTotalData(RulesLocationData?.data?.tableLocData);
        setTableLocData(RulesLocationData?.data?.tableLocData);
        const temp = RulesLocationData?.data?.tableLocData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter(row => row !== undefined);
        setcurrentPageData(temp);
        setcurrentPageRows(temp);
        setLoadCheck(false)
        if (RulesLocationData?.data?.tableLocData.length > 0) {
        ...
        setAllData(RulesLocationData?.data?.tableLocData);
    ...
    /* RL TEMPLATE */
      ...
      RulesLocationData?.data?.ruleTemplateNameData
      && Array.isArray(RulesLocationData?.data?.ruleTemplateNameData)
    ) {
      setRuleTemplateNameData(RulesLocationData?.data?.ruleTemplateNameData);
      setLoading(false);
    } else if (
      RulesLocationData?.data?.locTemplateNameData
      && Array.isArray(RulesLocationData?.data?.locTemplateNameData)
    ) {
      setLocTemplateNameData(RulesLocationData?.data?.locTemplateNameData);
      setLoading(false);
    }
    /* RL TEMPLATE */
    else {
  ...
  function getDayOfWeek(year, month, day) {
    ...
    return (dayOfWeek + 6) % 7; // Normalize to [0, 6] (0 = Saturday, 1 = Sunday, ..., 6 = Friday)
  }

  function adjustDate(inputDateStr) {
    let [year, month, day] = inputDateStr.split('-').map(Number);
    let targetDayOfWeek = 6; // Target day of the week (e.g., 6 for Saturday)
    let currentDayOfWeek = getDayOfWeek(year, month, day);
    let diff = targetDayOfWeek - currentDayOfWeek;

    if (diff > 0) {
      // Move forward by the number of days needed to reach the target day
      day += diff;
    } else if (diff < 0) {
      // Move forward by the number of days needed to reach the target day, considering wrapping around a week
      day += diff + 7;
    }

    // Normalize day and month
    while (true) {
      let daysInMonth = new Date(year, month, 0).getDate(); // Get days in the month
      if (day > daysInMonth) {
        day -= daysInMonth;
        month++;
        if (month > 12) {
  ...
  const sharedInputClassDisGird = {
    fontSize: '12px',
    height: '22px',
    backgroundColor: "#f0f0f0",
    // boxShadow: "rgba(0, 0, 0, 0.19) 0px 1px 2px, rgba(0, 0, 0, 0.23) 0px 2px 2px",
    borderRadius: "2px", margin: "0px"
    // marginBottom: "5px"
  };
  ...
  const onTableCellDateChange = (name, date, value) => {
    console.log("handleDatePicker:1:1 : ", name, date, value)
    if (date === null || date === "") {
      totalData.map((row) => { if (row.LOC === value) { row["RELEASE_DATE"] = "" } })
      tableLocData.map((row) => { if (row.LOC === value) { row["RELEASE_DATE"] = "" } })
      allData.map((row) => { if (row.LOC === value) { row["RELEASE_DATE"] = "" } })
      currentPageData.map((row) => { if (row.LOC === value) { row["RELEASE_DATE"] = "" } })
      currentPageRows.map((row) => { if (row.LOC === value) { row["RELEASE_DATE"] = "" } })
      // return;
    } else {
      ...
      totalData.map((row) => { if (row.LOC === value) { row["RELEASE_DATE"] = formattedDate } })
      tableLocData.map((row) => { if (row.LOC === value) { row["RELEASE_DATE"] = formattedDate } })
      allData.map((row) => { if (row.LOC === value) { row["RELEASE_DATE"] = formattedDate } })
      currentPageData.map((row) => { if (row.LOC === value) { row["RELEASE_DATE"] = formattedDate } })
      currentPageRows.map((row) => { if (row.LOC === value) { row["RELEASE_DATE"] = formattedDate } })
    }
    setSampleVal([]);
  }

  console.log("handleDatePicker:55 : ", totalData)
  ...
  const handleDateErase = (label) => {
    const dateSetters = {
      ...
      "START_DATE1": () => {
        setLeftContData(prev => ({ ...prev, START_DATE1: "" }));
      },
      "START_DATE2": () => {
        setLeftContData(prev => ({ ...prev, START_DATE2: "" }));
      },
      "END_DATE1": () => {
        setLeftContData(prev => ({ ...prev, END_DATE1: "" }));
      },
      "END_DATE2": () => {
        setLeftContData(prev => ({ ...prev, END_DATE2: "" }));
      },
      "ON_ORDER_COMMIT_DATE": () => {
  ...
  const handleDatePicker = (name, date) => {
    ...
    const day = String(Cdate.getDate()).padStart(2, '0');
    const formattedDate = `${year}-${month}-${day}`;
    const adjDate = adjustDate(formattedDate);
    switch (name) {
      case "START_DATE1":
        setLeftContData((prev) => {
          return {
      ...
      case "ON_ORDER_COMMIT_DATE":
        setLeftContData((prev) => {
          return {
            ...prev,
            ON_ORDER_COMMIT_DATE: formattedDate,
          };
        });
        break;
      default:
  ...
  const handleswitchRulecheck = (e, val) => {
    ...
    if (e.target.name === "START_DATE1") {
      ...
      if (e.target.value) {
        let newDateStr = adjustDate(e.target.value);
        setLeftContData((prev) => {
          return {
            ...prev,
            START_DATE1: newDateStr,
  ...
  console.log("ON_ORDER_COMMIT_WEEKS:::1::", leftContData);
  ...
  const selectAllocateTo = (val) => {
    setLeftContData(prev => ({
      ...
      NET_NEED_IND: val
        ? val.CODE : "",
      NET_NEED_IND_VAL: val
        ? val.CODE_DESC : ""
    }));
    if (val) {
      setLeftContData(prev => ({ ...prev, INCLUDE_INV_IN_MIN_IND: val.CODE === 'N' ? "N" : "Y", INCLUDE_INV_IN_MAX_IND: val.CODE === 'N' ? "N" : "Y" }))
    } else {
      setLeftContData(prev => ({ ...prev, INCLUDE_INV_IN_MIN_IND: "Y", INCLUDE_INV_IN_MAX_IND: "Y" }))
    }
  }
  ...
  if (Object.keys(AllocRuleData).length > 0 && RetrieveDataCheck && ruleType.length > 0
    ...
    if (String(AllocRuleData["START_DATE1"]).length > 0 || String(AllocRuleData["END_DATE1"]).length > 0 ||
      ...
      if (String(AllocRuleData["START_DATE1"])) {
        setLeftContData((prev) => { return { ...prev, START_DATE1: String(AllocRuleData["START_DATE1"]).length > 0 ? String(AllocRuleData["START_DATE1"]) : "" }; })
      }

      if (String(AllocRuleData["END_DATE1"])) {
        setLeftContData((prev) => { return { ...prev, END_DATE1: String(AllocRuleData["END_DATE1"]).length > 0 ? String(AllocRuleData["END_DATE1"]) : "" }; })
      }

      if (String(AllocRuleData["START_DATE2"])) {
        setLeftContData((prev) => { return { ...prev, START_DATE2: String(AllocRuleData["START_DATE2"]).length > 0 ? String(AllocRuleData["START_DATE2"]) : "" }; })
      }

      if (String(AllocRuleData["END_DATE2"])) {
        setLeftContData((prev) => { return { ...prev, END_DATE2: String(AllocRuleData["END_DATE2"]).length > 0 ? String(AllocRuleData["END_DATE2"]) : "" }; })
      }
    }
    ...
    if (String(AllocRuleData["ON_ORDER_COMMIT_DATE"])) {
      setLeftContData((prev) => { return { ...prev, ON_ORDER_COMMIT_DATE: String(AllocRuleData["ON_ORDER_COMMIT_DATE"]).length > 0 ? String(AllocRuleData["ON_ORDER_COMMIT_DATE"]) : "" }; })
    ...
    if (String(AllocRuleData["INCLUDE_INV_IN_MIN_IND"])) {
      setLeftContData((prev) => { return { ...prev, INCLUDE_INV_IN_MIN_IND: String(AllocRuleData["INCLUDE_INV_IN_MIN_IND"]).length > 0 ? String(AllocRuleData["INCLUDE_INV_IN_MIN_IND"]) : "" }; })
    }
    if (String(AllocRuleData["INCLUDE_INV_IN_MAX_IND"])) {
      setLeftContData((prev) => { return { ...prev, INCLUDE_INV_IN_MAX_IND: String(AllocRuleData["INCLUDE_INV_IN_MAX_IND"]).length > 0 ? String(AllocRuleData["INCLUDE_INV_IN_MAX_IND"]) : "" }; })
    }
    if (String(AllocRuleData["CASCADE_IND"])) {
      setLeftContData((prev) => { return { ...prev, CASCADE_IND: String(AllocRuleData["CASCADE_IND"]).length > 0 ? String(AllocRuleData["CASCADE_IND"]) : "" }; })
    }
    if (String(AllocRuleData["USE_RULE_LEVEL_ON_HAND_IND"])) {
      setLeftContData((prev) => { return { ...prev, USE_RULE_LEVEL_ON_HAND_IND: String(AllocRuleData["USE_RULE_LEVEL_ON_HAND_IND"]).length > 0 ? String(AllocRuleData["USE_RULE_LEVEL_ON_HAND_IND"]) : "" }; })
    }
    if (String(AllocRuleData["INCLUDE_CLEARANCE_STOCK_IND"])) {
      setLeftContData((prev) => { return { ...prev, INCLUDE_CLEARANCE_STOCK_IND: String(AllocRuleData["INCLUDE_CLEARANCE_STOCK_IND"]).length > 0 ? String(AllocRuleData["INCLUDE_CLEARANCE_STOCK_IND"]) : "" }; })
    }
    if (String(AllocRuleData["TEMPLATE_NO"]).length > 0) {
      const temp = ruleTemplateNameData.filter(item => item.TEMPLATE_NO === String(AllocRuleData["TEMPLATE_NO"]))
      if (temp.length > 0) {
        setLeftContData((prev) => { return { ...prev, TEMPLATE_NO: temp[0].TEMPLATE_NO }; })
      }
    }
  ...
  const SearchButtonReleaseDate = () => (
    [<IconButton sx={{ padding: "0px 3px 0px 0px", margin: "0px" }} disabled={ApproveFreeseCheck}>
  ...
  const handleChangeWeight = (e, name) => {
    if (e) {
      ...
      if (e.target.name === "RELEASE_DATE") { setCopyValue((prev) => ({ ...prev, [e.target.name]: e.target.value })) }
    }
  }
  ...
  const handleChangeWeight1 = (e, name) => {
    if (e) {
      ...
      if (e.target.name === "RELEASE_DATE") { setInputValue1((prev) => ({ ...prev, [e.target.name]: e.target.value })) }
    }
  }
  const handleChangeReleaseDate = (name, date) => {
    if (date === null || date === "") {
      setCopyValue((prev) => {
        return {
          ...prev,
      ...
      const day = String(Cdate.getDate()).padStart(2, '0');
      const formattedDate = `${year}-${month}-${day}`;
      setCopyValue((prev) => ({ ...prev, ["RELEASE_DATE"]: formattedDate }))
    }
  }

  const handleChangeReleaseDate1 = (name, date) => {
    if (date === null || date === "") {
      setInputValue1((prev) => {
        return {
          ...prev,
      ...
      const day = String(Cdate.getDate()).padStart(2, '0');
      const formattedDate = `${year}-${month}-${day}`;
      setInputValue1((prev) => ({ ...prev, ["RELEASE_DATE"]: formattedDate }))
    }
  }
  ...
  const HandleSaveChanges = () => {
    dispatch(getUPDATECHANGEWEIGHTSRLRequest(RetrieveChangeWeightsRL));
  ...
  return (
    <Box >
      ...
      <div className={RulesLocationHeaderClasses.course_box}>
        ...
        <Paper
          sx={{ margin: "4px 0px 0px 5px", mb: 0, width: "100%", height: "auto", borderRadius: 1, boxShadow: 2, border: 0, borderBottom: 3, }} //width: "calc(100% - 0px)", 
        >
          ...
          <Box display="flex" justifyContent="flex-end" sx={{ width: "99.4%", margin: "0px 0px 0px 5px", }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              ...
              >
                <ViewColumnIcon style={{
                  padding: "0px", backgroundColor: isSHovered1 ? '#f5f5f5' : 'white',
                  marginTop: "2px", color: "DodgerBlue"
                }}
                  onClick={HandleManageHeader}
                  title="Manage Columns" />
              </div>
                  ...
                  {ManageHeaderData.includes('LOC') ?
                    <TableCell sx={{ padding: "0px" }} className={RulesLocationHeaderClasses.TitleHead}>
                      <TextField
                        ...
                        sx={{ width: "100%" }}
                        variant="standard"
                        inputProps={{ sx: { padding: "0px", height: "20px", textAlign: "center", } }}
                        disabled={LockCheck}
                      />
                    </TableCell> : null}
                  ...
                  {ManageHeaderData.includes('RELEASE_DATE') ?
                    <TableCell sx={{ padding: "0px" }}>
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => LockCheck ? handleChangeReleaseDate("RELEASE_DATE", date) : handleChangeReleaseDate1("RELEASE_DATE", date)}
                        onChangeRaw={(event) => {
                          event.preventDefault();
                        }}
                        dateFormat="MM-dd-yy"
                        ...
                        customInput={
                          <TextField
                            ...
                            autoComplete='off'
                            helperText=""
                            // sx={{ "& .MuiInputBase-input.Mui-disabled": { backgroundColor: "#f0f0f0", height: "10px", borderRadius: "5px" }, }}
                            id="outlined-disabled"
                            InputLabelProps={{ style: { fontSize: "12px", }, shrink: true, }}
                            InputProps={{
                              style: (ApproveFreeseCheck) ? sharedInputClassDisGird : sharedInputClassGrid,
                              endAdornment: (<SearchButtonReleaseDate />)
                              // endAdornment: (<>
                              //   <CalendarTodayIcon style={{ fontSize: "11px", margin: "0px 3px 0px 0px" }} />
                              //   {(copyValue?.RELEASE_DATE?.length > 0 || inputValue1?.RELEASE_DATE?.length > 0 )
                              //     && <BsFillEraserFill fontSize="medium"
                              //       onClick={() => handleDateErase("GRID")} />}
                              // </>)
                            }}
                            sx={{
                              minWidth: "130px", "& input::placeholder": { fontSize: "10px", }, border: 0, borderBottom: "1px solid gray",
                                    ...
                                    InputProps={{
                                      ...
                                      endAdornment: (<>
                                        ...
                                        {(row.RELEASE_DATE.length > 0)
                                          && <BsFillEraserFill fontSize="medium"
                                            onClick={() => onTableCellDateChange("RELEASE_DATE", '', row.LOC)} />}
                                      </>)
                                    }}
                                    sx={{ "& input::placeholder": { fontSize: "10px", } }}
                                    inputProps={{ style: { fontSize: "12px" }, placeholder: "MM-DD-YY", }}
                                    disabled={ApproveFreeseCheck}
                                  />
                                }
                          ...
                          .map((row, index) => {
                            return (
                              <TableRow hover role="checkbox" tabIndex={-1} key={row.EOW}>
                                <TableCell align="right" sx={{ fontFamily: "system-ui", textAlign: "center", fontSize: "75%", padding: 0 }}>
                                  {convertDateFormat(row.EOW_DATE)}
                                </TableCell>
                                ...
                                <TableCell align="right" sx={{ fontFamily: "system-ui", textAlign: "left", padding: 0 }} textAlign="right">
                                  <TextField
                                    ...
                                    autoComplete="off"
                                    inputProps={{ maxLength: 3, sx: { backgroundColor: '#fff', padding: "0px", height: "30px", textAlign: "center", } }}
                                    disabled={ApproveFreeseCheck}
                                  />
                                </TableCell>
                              </TableRow >
      ...
      <div>
        <Dialog
          fullWidth={true}
          maxWidth="xs"
          open={ruleTemplateCheck}
          PaperComponent={PaperComponent}
          aria-labelledby="draggable-dialog-title"
          disableBackdropClick
        >
          <DialogTitle id="draggable-dialog-title" sx={{ padding: "5px 0px 0px 13px", margin: "0px", fontsize: "12px" }}>
            Rules Template Name:
          </DialogTitle>
          <DialogContent sx={{ padding: "0px 0px 0px 0px", margin: "0px", }}>
            <Box
              component="fieldset"
              display="flex"
              flexDirection="column"
              justifyContent={"space-between"}
              sx={{ borderRadius: 2, border: 0, paddingBottom: '0px', }}
            >
              <TextField
                size="small"
                variant="outlined"
                onBlur={handleRuleTemplateName}
                sx={{
                  "& .MuiInputBase-input.Mui-disabled": { backgroundColor: "#f0f0f0", borderRadius: "5px", },
                  input: { "&::placeholder": { opacity: 1, }, },
                  border: 0
                }}
                InputLabelProps={{ style: { fontSize: "12px" }, shrink: "true" }}
                InputProps={{ style: { fontSize: 15, }, }}
                inputProps={{ maxLength: 200, sx: { backgroundColor: '#fff' } }}
              />
            </Box>
          </DialogContent>
          <DialogActions>
            <Button
              sx={{ fontSize: "12px", backgroundColor: "#228B22", padding: "5px", fontFamily: "system-ui", width: "100px", marginLeft: "5px", marginTop: "2px", }}
              startIcon={<DoneAllIcon />}
              onClick={RulesSaveTemplateOk}
              variant="contained">
              Ok
            </Button>
            <Button
              sx={{ backgroundColor: "maroon", fontSize: "12px", padding: "5px", fontFamily: "system-ui", width: "100px", marginLeft: "5px", marginTop: "2px", }}
              startIcon={<CancelIcon />}
              variant="contained"
              onClick={() => {
                setRuleTemplateName("");
            ...
            >
              ...
              <div>
                <TextField
                  ...
                  size="small"
                  variant="outlined"
                  onBlur={handleLocInputTemplateData}
                  sx={{
                    "& .MuiInputBase-input.Mui-disabled": { backgroundColor: "#f0f0f0", borderRadius: "5px", },
                    input: { "&::placeholder": { opacity: 1, }, },
                    border: 0
