...
const DataProcessing = () => {
    ...
    useEffect(() => {
        if (ProcessData?.data?.insH1Status?.status === 201) {
            ...
            ProcessData.data.insH1Status.status = 0;
        } else if (ProcessData?.data?.insH2Status?.status === 201) {
            setIsLoading(false);
            setOpenDialog2(true);
            setDialogData2(ProcessData?.data?.insH2Status?.message)
            ProcessData.data.insH2Status.status = 0;
        } else if (ProcessData?.data?.insH2Status?.status === 500) {
            setIsLoading(false);
            setOpenDialog2(true);
            setDialogData2(ProcessData?.data?.insH2Status?.message)
            setReloadStatus(true);
            ProcessData.data.insH2Status.status = 0;
        } else if (ProcessData?.data?.insH3Status?.status === 201) {
            setIsLoading(false);
            setOpenDialog2(true);
            setDialogData2(ProcessData?.data?.insH3Status?.message)
            ProcessData.data.insH3Status.status = 0;
        } else if (ProcessData?.data?.insH3Status?.status === 500) {
            setIsLoading(false);
            setOpenDialog2(true);
            setDialogData2(ProcessData?.data?.insH3Status?.message)
            setReloadStatus(true);
            ProcessData.data.insH3Status.status = 0;
        } else if (ProcessData?.data?.insTtmDtlStatus?.status === 201) {
            setIsLoading(false);
            setOpenDialog2(true);
            setDialogData2(ProcessData?.data?.insTtmDtlStatus?.message)
            ProcessData.data.insTtmDtlStatus.status = 0;
        } else if (ProcessData?.data?.insTtmDtlStatus?.status === 500) {
            setIsLoading(false);
            setOpenDialog2(true);
            setDialogData2(ProcessData?.data?.insTtmDtlStatus?.message)
            setReloadStatus(true);
    ...
    const handleCapture = ({ target }) => {
        ...
        ExcelRenderer(fileObj, (err, resp) => {
            ...
            const dataSet = [file_Columns, ...adjustedRows].filter(subArray => 
                !subArray.every(value => value === "")
            );
            
            // console.log("dataSet", dataSet,adjustedRows);
    ...
    const finalSubmit = () => {
        ...
        if (processName === "HIER1") {
            dispatch(postSEEDHIER1Request([{
                TABLE_DATA: tableData,
                USER: currentUser
            }]));
            console.log("process_name1", processName)
        } else if (processName === "HIER2") {
            dispatch(postSEEDHIER2Request([{
                TABLE_DATA: tableData,
                USER: currentUser
            }]));
            console.log("process_name2", processName)
        } else if (processName === "HIER3") {
            dispatch(postSEEDHIER3Request([{
                TABLE_DATA: tableData,
                USER: currentUser
            }]));
            console.log("process_name3", processName)
        } else if (processName === "ITEM_DTL") {
            dispatch(postSEEDITMDTLRequest([{
                TABLE_DATA: tableData,
                USER: currentUser
            }]));
            console.log("process_name4", processName)
        }
    }
    return (
