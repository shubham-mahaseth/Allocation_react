...
const AllocSummary = () => {
  ...
  */
    ...
    useEffect(() => {
        ...
        if (AllocSummaryData?.data?.hierData && Array.isArray(AllocSummaryData?.data?.hierData)) {
            let UniqDept =
                AllocSummaryData?.data?.hierData.length > 0
                    ? [...new Map(AllocSummaryData?.data?.hierData.map((item) => [item["HIER1"], item])).values()]
                    : [];
            setHier1Data(UniqDept);
        } else if (AllocSummaryData?.data?.hier2Data && Array.isArray(AllocSummaryData?.data?.hier2Data)) {
        ...
        } else if (AllocSummaryData?.data?.poData && Array.isArray(AllocSummaryData?.data?.poData)) {
            let UniqPO =
                AllocSummaryData?.data?.poData.length > 0
                    ? [...new Map(AllocSummaryData?.data?.poData.map((item) => [item["PO"], item])).values()]
                    : [];
            setPOData(UniqPO);
        } else if (AllocSummaryData?.data?.tsfData && Array.isArray(AllocSummaryData?.data?.tsfData)) {
            let UniqTSF =
                AllocSummaryData?.data?.tsfData.length > 0
                    ? [...new Map(AllocSummaryData?.data?.tsfData.map((item) => [item["TSF"], item])).values()]
                    : [];
            setTSFData(UniqTSF);
        ...
        } else if (AllocSummaryData?.data?.asnData && Array.isArray(AllocSummaryData?.data?.asnData)) {
            let UniqASN =
                AllocSummaryData?.data?.asnData.length > 0
                    ? [...new Map(AllocSummaryData?.data?.asnData.map((item) => [item["ASN"], item])).values()]
                    : [];
            setASNData(UniqASN);
            ...
            AllocSummaryData?.data?.packNoData &&
            Array.isArray(AllocSummaryData?.data?.packNoData)
        ) {
            setPackNoData(AllocSummaryData?.data?.packNoData);
        } else if (
            AllocSummaryData?.data?.diffData &&
            Array.isArray(AllocSummaryData?.data?.diffData)
        ) {
            setDIffData(AllocSummaryData?.data?.diffData);
        } else if (
            AllocSummaryData?.data?.skuData &&
            Array.isArray(AllocSummaryData?.data?.skuData)
        ) {
            setSkuData(AllocSummaryData?.data?.skuData);
        } else if (
            AllocSummaryData?.data?.itemParentData &&
            Array.isArray(AllocSummaryData?.data?.itemParentData)
        ) {
            setItemParentData(AllocSummaryData?.data?.itemParentData);
        } else if (
            AllocSummaryData?.data?.contextTypeData &&
            Array.isArray(AllocSummaryData?.data?.contextTypeData)
        ) {
            setContextTypeData(AllocSummaryData?.data?.contextTypeData);
        } else if (
            AllocSummaryData?.data?.promotionData &&
            Array.isArray(AllocSummaryData?.data?.promotionData)
        ) {
            setPromotionData(AllocSummaryData?.data?.promotionData);
        } else if (
            AllocSummaryData?.data?.criteriaData &&
            Array.isArray(AllocSummaryData?.data?.criteriaData)
        ) {
            setCriteriaData(AllocSummaryData?.data?.criteriaData);
        }

        if (
            AllocSummaryData1?.data?.alltrSts &&
            Array.isArray(AllocSummaryData1?.data?.alltrSts)
        ) {
            setAlltr((AllocSummaryData1?.data?.alltrSts)[0]);
            setAStatus((AllocSummaryData1?.data?.alltrSts)[1]);
        } else if (
            AllocSummaryData1?.data?.allocIdsData &&
            Array.isArray(AllocSummaryData1?.data?.allocIdsData)
        ) {
            if (initSrch) {
            ...
            AllocSummaryData1?.data?.srcData &&
            Array.isArray(AllocSummaryData1?.data?.srcData)
        ) {
            ...
            AllocSummaryData1?.data?.validCheck &&
            Array.isArray(AllocSummaryData1?.data?.validCheck)
        ) {
            setCheck(true);
            setIsLoadingAllocSumm(false);
            ...
            AllocSummaryData1?.data?.copyAsyData &&
            Array.isArray(AllocSummaryData1?.data?.copyAsyData)
        ) {
            if ((AllocSummaryData1?.data?.copyAsyData).length > 0) {
                ...
                swal({
                    ...
                    // call Validation for RLCheck
                    dispatch(postValidASYRequest([{ "ALLOC_NO": (AllocSummaryData1?.data?.copyAsyData)[0] }]));

                });
            }
            //setRLCheck((AllocSummaryData1?.data?.copyAsyData)[0])
        }
    ...
    */
        ...
        switch (name) {
            case "RELEASE_DATE_FROM":
                setSearchData((prev) => {
                    return {
                        ...prev,
                        RELEASE_DATE_FROM: date === null ? "" : date,
                    };
                });
                break;
            ...
            case "CREATE_DATE_FROM":
                setSearchData((prev) => {
                    return {
                        ...prev,
                        CREATE_DATE_FROM: date === null ? "" : date,
                    };
                });
                break;
            case "CREATE_DATE_TO":
    ...
    const [isAllocAttrExpanded, setAllocAttrExpanded] = useState(true);
    const [isHierExpanded, setHierExpanded] = useState(true);
    const [isAttrExpanded, setAttrExpanded] = useState(false);
    const [isSelExpanded, setSelExpanded] = useState(true);
                                ...
                                <div className={AllocDetailsClasses.float_child}>
                                    ...
                                    <div>
                                        ...
                                        <DatePicker
                                            ...
                                            selected={searchData.RELEASE_DATE_FROM}
                                            onChange={(date) => handleDateChangePicker("RELEASE_DATE_FROM", date)}
                                            onChangeRaw={(event) => {
                                                //console.log(validateDateInput(event.target.value), typeof event.target.value)
                                                if (validateDateInput(event.target.value)) {
                                                    //   //console.log("Valid date format");                              
                                                    // Additional logic for handling the valid date input
                                                } else {
                                                    setOpenDialogQL(true);
                                                    setDialogDataQL("Invalid Release Date From Input");
                                ...
                                <div className={AllocDetailsClasses.float_child}>
                                    ...
                                    <div>
                                        <DatePicker
                                            autoComplete="off"
                                            placeholderText="MM-DD-YY"
                                            selected={searchData.RELEASE_DATE_TO}
                                            ...
                                            onChangeRaw={(event) => {
                                                if (validateDateInput(event.target.value)) {
                                                    ...
                                                    setOpenDialogQL(true);
                                                    setDialogDataQL("Invalid Release Date To Input");
                                                    // Additional logic for handling the invalid date input
                                                }
                                            }}
                                            dateFormat="MM-dd-yy"
                                ...
                                <div className={AllocDetailsClasses.float_child}>
                                    ...
                                    <div>
                                        <DatePicker
                                            autoComplete="off"
                                            placeholderText="MM-DD-YY"
                                            selected={searchData.CREATE_DATE_FROM}
                                            ...
                                            onChangeRaw={(event) => {
                                                if (validateDateInput(event.target.value)) {
                                                    ...
                                                    setOpenDialogQL(true);
                                                    setDialogDataQL("Invalid Create Date From Input");
                                                    // Additional logic for handling the invalid date input
                                                }
                                            }}
                                            dateFormat="MM-dd-yy"
                                ...
                                <div className={AllocDetailsClasses.float_child}>
                                    ...
                                    <div>
                                        <DatePicker
                                            autoComplete="off"
                                            placeholderText="MM-DD-YY"
                                            selected={searchData.CREATE_DATE_TO}
                                            ...
                                            onChangeRaw={(event) => {
                                                if (validateDateInput(event.target.value)) {
                                                    ...
                                                    setOpenDialogQL(true);
                                                    setDialogDataQL("Invalid Create Date To Input");
                                                    // Additional logic for handling the invalid date input
                                                }
                                            }}
                                            dateFormat="MM-dd-yy"
            ...
            >
                {tabData.length > 0 && (
                    <Typography
                        ...
                        component="div"
                    >
                        Rows {selected.length} of {tabData.length}
                    </Typography>
                )}
            </Toolbar>
        );
