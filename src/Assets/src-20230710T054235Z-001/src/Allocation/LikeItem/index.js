...
const LikeItemMap = ({ allocNoData, tab, setTab, setRTabCond, setDisCond, limData, setLIMData, ApproveFreeseCheck, setHeaderCheck,
  ...
  const map_head = [
    ...
    { id: "MAP_SIZE_PR", label: "Map" + "\u00A0" + "Size" + "\u00A0" + "Pr" }
  ]
  ...
  useEffect(() => {
    if (limData.length > 0 && !AllocationData?.data?.Data && (tableData.length === 0 && mapTableData.length === 0)) {
      if (limData[0].length > 0) {
        setTableData(serializedata(limData[0]));
        setSelectedRow(1);
      } else {
        setTableData([]);
      }
      if (limData[1].length > 0) {
        setMapTableData(serializedataMap(limData[1]));
        setSelectedMapRow(1);
      } else {
        setMapTableData([]);
      }
  ...
  useEffect(() => {
    ...
    if (AllocationData?.data?.hierData && Array.isArray(AllocationData?.data?.hierData)) {
      ...
      if (((AllocationData?.data?.likeItemTableData)[0]).length > 0) {
        setTableData(serializedata((AllocationData?.data?.likeItemTableData)[0]));
        setSelectedRow(1);
      } else {
        setTableData([]);
      }
      if (((AllocationData?.data?.likeItemTableData)[1]).length > 0) {
        setMapTableData(serializedataMap((AllocationData?.data?.likeItemTableData)[1]));
        setSelectedMapRow(1);
      } else {
        setMapTableData([]);
      }
      ...
      if (((AllocationData?.data?.Data)[0]).length > 0) {
        setTableData(serializedata((AllocationData?.data?.Data)[0]));
        setSelectedRow(1);
      } else {
        setTableData([]);
      }
      if (((AllocationData?.data?.Data)[1]).length > 0) {
        setMapTableData(serializedataMap((AllocationData?.data?.Data)[1]));
        setSelectedMapRow(1);
      } else {
        setMapTableData([]);
      }
  ...
  */
  ...
  const handleMap = () => {
    ...
    if (allocHDetails[0].ALLOC_LEVEL_CODE === "T") {
      ...
      setSelected([]);
      */
      const temp = [...tableData.filter(obj => selected.includes(obj.SR_NO))];
  ...
  const handleDelete = () => {
    ...
    */
    const temp = [...mapTableData.filter(obj => selectedMap.includes(obj.SR_NO))];
    var delMapData = {}
...
*/
  const handleSelectAllClick = (event) => {
    if (event.target.checked && selected.length === 0) {
      const newSelected = tableData.map((n) => n.SR_NO);
  ...
  const handleMapSelectAllClick = (event) => {
    if (event.target.checked && selectedMap.length === 0) {
      const newSelected = mapTableData.map((n) => n.SR_NO);
      ...
      <TableHead className={LikeItem.TitleHead}>
        <TableRow className={LikeItem.TitleRow}
          ...
          {map_head.map((headCell) => (ManageHeaderData.includes(headCell.id) &&
            ...
            >
              {['WEIGHT%', "MAP_SIZE_PR"].includes(headCell.id) ? headCell.label :
                <TableSortLabel
    ...
    <Box >
      ...
      <Box component="fieldset"
        ...
        >
          ...
          >
            <legend style={{ fontWeight: "bold", color: "#191970", padding: "5px 0px 0px 5px" }}>Allocated Items </legend>
              ...
              >
                <Table
                  ...
                  <TableBody >
                        ...
                        .map((row, index) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <TableCell align="left" style={{ padding: "0px 0px 0px 3px", textAlign: "left", fontSize: "12px", width: "70px", whiteSpace: "nowrap", overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100px' }}>
                              ...
                              <TableCell  //sx={{ fontFamily: "system-ui", fontSize: "75%", padding: 0,justifyContent: "left"}} 
                                ...
                                sx={{ padding: "0px 0px 0px 3px", textAlign: "left", fontSize: "12px", whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100px', paddingLeft: "2px", }}
                              >
                              ...
                              <TableCell //align="right" sx={{ fontFamily: "system-ui", textAlign: "left", fontSize: "75%", padding: 0 }} textAlign="right"
                                sx={{ padding: "0px 0px 0px 3px", textAlign: "left", fontSize: "12px", width: "70px" }}
                              >
                                <Box
                                  display="flex"
                                  justifyContent="space-between"
                    ...
                    {tableData.length < 20 ?
                      [...Array(20 - (tableData.length)).keys()].map(val => (
                        <TableRow >
          ...
          <Box gridColumn="span 8">
            <legend style={{ fontWeight: "bold", color: "#191970", backgroundColor: "", display: "flex", justifyContent: "space-between" }}>
              ...
              {/* // <Box display="inline" justifyContent="flex-end" sx={{}}> */}
              {/* <Button
                autoFocus
                variant="contained"
              ...
              >
                <Table //</TableContainer>sx={{ minWidth: "auto", }} 
                  ...
                  <TableBody >
                          ...
                          return (
                            ...
                            >
                              ...
                              {ManageHeaderData.includes('DIFF_ID') ?
                                <TableCell sx={{ padding: "0px 0px 0px 3px", textAlign: "left", fontSize: "12px", whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100px' }}
                                >
                                  <Box
                                    display="flex"
                                    justifyContent="space-between"
                              ...
                              {ManageHeaderData.includes('LIKE_ITEM_DIFF_ID') ?
                                <TableCell
                                  sx={{
                                    padding: "0px 0px 0px 3px", textAlign: "left", fontSize: "12px",
                              ...
                              {ManageHeaderData.includes('WEIGHT%') ?
                                <TableCell align="left" style={{
                                  padding: "0px 0px 0px 3px", textAlign: "center",
                              ...
                              {ManageHeaderData.includes('MAP_SIZE_PR') ?
                                <TableCell padding="checkbox" sx={{ padding: "0px", }}>
                                  <Checkbox style={{ padding: "0px", textAlign: "center", display: 'flex', justifyContent: 'center', alignItems: 'center', }} color="primary" size="small" disabled={allocHDetails.length > 0 ? allocHDetails[0].ALLOC_LEVEL_CODE === "T" : false} />
                                </TableCell> : null}
                            </TableRow>
                          );
                        })
                    ...
                    {mapTableData.length < 20 ?
                      [...Array(20 - (mapTableData.length)).keys()]
                        .map(val => (
                          <TableRow
                          // sx={{'&.MuiTableRow-root': {
