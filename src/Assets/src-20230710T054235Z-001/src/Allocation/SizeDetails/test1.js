...
const SizeDetails = () => {
    ...
    useEffect(() => {
        if (
            ...
            setSizeHeaderDetailsData([]);
            setTableSizeHeaderDetailsData([]);
            setSizeHeaderDetailsData(SizeDetailsData?.data?.sizeHeaderDetailsData);
            setTableSizeHeaderDetailsData(SizeDetailsData?.data?.sizeHeaderDetailsData);
            var check = false;
        ...
        if (
            ...
            // setSizeDetailsData(SizeDetailsData?.data?.sizeDetailsData)
            setSizeDetailsData1(SizeDetailsData?.data?.sizeDetailsData)

            const orderedObj = {};
            for (const key in SizeDetailsData?.data?.sizeDetailsData[0]) {
                if (key === 'DIFF_ID') {
                    orderedObj[key] = SizeDetailsData?.data?.sizeDetailsData[0][key];
                    orderedObj['TOTAL_SKU_CALC_QTY'] = SizeDetailsData?.data?.sizeDetailsData[0]['TOTAL_SKU_CALC_QTY'];
            ...
            // delete newObject.ALLOC_NO
            // delete newObject.ASSIGN_DEFAULT_WH
            // delete newObject.ORDER_NO
            // delete newObject.TO_LOCATION

            const headColumns = Object.keys(SizeDetailsData?.data?.sizeDetailsData[0])
            ...
            setHeadList(headColumns);

            const temp_list100 = []

            SizeDetailsData?.data?.sizeDetailsData.map(row => {
                const orderedObj = {};
                ...
                temp_list100.push(newObject)
            })

            setSizeDetailsData([SizeDetailsData?.data?.sizeDetailsData[0]])

            console.log("UniqsizeDetailsDataLOC: ", SizeDetailsData?.data?.sizeDetailsData, headColumns, orderedObj, modifiedList, newObject, temp_list100);

            const result = [];

            for (const key in SizeDetailsData?.data?.sizeDetailsData[0]) {
                if (SizeDetailsData?.data?.sizeDetailsData[0].hasOwnProperty(key)) {
                    const parts = key.split('_');
                    if (parts.length > 2) {
                        result.push(parseInt(parts[2]));
                    }
                }
            }

            console.log("UniqsizeDetailsDataLOC: 2", SizeDetailsData?.data?.sizeDetailsData, result);
    ...
    console.log("SizeDetailsData:6: ", sizeTempData, SizeDetailsData);
            ...
            <div>
                <TableContainer component={Paper} style={{ maxHeight: 280, width: "calc(92vw - 0px)" }}>
                    <Table className={classes.table} aria-label="simple table">
                        ...
                        <TableBody>
                            {sizeDetailsData.map((row) => {
                                return (
                                    <TableRow key={row.SOURCE_ITEM}>
                                        {Object.keys(row).map((key, index) => {
                                            return (
                                                <TableCell key={index} sx={{ padding: "0px", textAlign: "left", fontSize: "12px" }}>
                                                    {/* {row[Object.keys(row)[index]]} */}
                                                    {row[key]}
                                                    {/* {console.log("TableCell:1: ",row[Object.keys(row)[index]],key,row[key])} */}
                                                </TableCell>
                                            )
                                        })}
                                    </TableRow>
